...
Order::Order(int idorder, const std::string& marketIdentificationCode, const std::string& tradingCurrency,
    ...
    : idorder(idorder),
      ...
      marketIdentificationCode(marketIdentificationCode), tradingCurrency(tradingCurrency), priority(priority),
      price(price), quantity(quantity), timeinforce(timeinforce),
      ordertype(ordertype), limitType(limitType), idinstrument(idinstrument), originalqty(originalqty), idfirm(idfirm),
